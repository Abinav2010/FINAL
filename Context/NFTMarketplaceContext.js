...
const connectingWithSmartContract = async () => {
  ...
  export const NFTMarketplaceProvider=({children}) => {
    const titleData ="Discover and Support the Creators";
 ...
 const connectWallet = async () => {
    try {
      ...
      const accounts = await window.ethereum.request({
        ...
        //const profilePicture = await web3.eth.getProfile({
        //method: 'avatar',
      //});

      setCurrentAccount(accounts[0]);
    } catch (error) {
        console.log("Error whhile connecting to wallet");
    }
  };
...
function calculateHash(input) {
  /**
   * Calculates the SHA256 hash value for the given input string.
   */
  const hasher = crypto.createHash('sha256');
  hasher.update(input);
  const digest = hasher.digest('hex');  
  if (digestList.includes(digest)) {
    // Duplicate digest found.
    return true;
  } else {
    // New digest, add to the list.
...
const createNFT = async (name, price, image, description, router) => {
  ...
  const data = JSON.stringify({ name, description, image, created: now });
  console.log(data);
...
const fetchNFTs = async () => {
  try {
      ...
      for (const { tokenId, seller, owner, price: unformattedPrice } of data) {
